...
| `/collections/chief-of-staff` | Compare five Chief of Staff Bot and team setups. |
...
check, and receive approval from the `@skillselion` brand code owner. That
administration account may bypass the pull-request and status-check rules or
push directly to `main` for its own changes. CI still runs after a direct push.
Branch deletion, force pushes, and nonlinear history cannot be bypassed.
