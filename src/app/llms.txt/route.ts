...
export function GET() {
  ...
  const body = [`# ${site.name}`, "", `> ${site.entity}`, "", `${site.company}. Contact: ${site.email}`, `Source: ${site.github}`, `Canonical domain: botteams.ai. Type-in grokbotteam.ai should 301 when DNS exists.`, "", "## Pages", "", `- [Home](${site.url})`, `- [What is Grok Bot?](${site.url}/grok-bot)`, `- [Guides](${site.url}/guides)`, `- [Connectors](${site.url}/connectors)`, `- [Docs](${site.url}/docs)`, `- [API](${site.url}/api)`, `- [Bots](${site.url}/?kind=bot#teams)`, `- [OpenAPI 3.1](${site.url}/openapi.json)`, `- [API: teams](${site.url}/api/teams)`, `- [API: bots](${site.url}/api/bots)`, `- [Sponsor](${site.url}/sponsor)`, "", "## Guides", "", "One query per URL. The definition stays on /grok-bot.", "", guideLines, "", "## Teams", "", "A team is two to six Bots in one group chat.", "", teamLines, "", "## Bots", "", "A bot is one Bot doing one job. No group chat.", "", botLines, ""].join("\n");
